# Support Ticket Classifier Evaluation with n8n
...
This workflow uses **Ollama Cloud** (hosted models), not a local Ollama installation:

- **API**: Ollama Cloud API (`https://ollama.com/api/chat`)
- **Model**: `nemotron-3-nano:30b-cloud` (cloud-hosted model)
- **Authentication**: Bearer token from Ollama Cloud, stored in `OLLAMA_BEARER_TOKEN` environment variable
  - Get your bearer token from: https://ollama.com
  - The token authenticates your n8n workflow to use Ollama's cloud-hosted models
...
**Note**: Even though you may run n8n locally, this workflow calls Ollama Cloud's hosted API, so you need an Ollama Cloud account and bearer token.
...
3. Set the `OLLAMA_BEARER_TOKEN` environment variable:
   - **For local n8n**: Copy `config.example` to `.env` and replace `your_bearer_token_here` with your actual Ollama API bearer token, then restart n8n
   - **For n8n Cloud/hosted**: Set `OLLAMA_BEARER_TOKEN` in your n8n instance's environment variables settings
   - The workflow uses `{{ $env.OLLAMA_BEARER_TOKEN }}` to read the token
...
- `Support ticket classifier - eval.json` - The n8n workflow file to import
- `Customer support ticket eval 1 - Sheet2.csv` - Sample evaluation data
- `config.example` - Example configuration file for environment variables
